...
const onRequest = <D>(config: AxiosRequestConfig<D>): AxiosRequestConfig<D> => {
  const csrfToken = Cookies.get("uua_csrf_token");
  config.headers = {
...
const onRequestError = <T, D>(error: AxiosError<T, D>): Promise<AxiosError<T, D>> => {
...
const onResponseError = <T, D>(error: AxiosError<API.BaseResponse<T>, D>): Promise<T> => {
